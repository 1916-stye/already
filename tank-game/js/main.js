(function(){var T=window.TankGame=window.TankGame||{},canvas=document.getElementById('gameCanvas'),input=new T.Input(canvas),game=new T.Game(canvas,input),selectedMode='normal';function start(){game.start(selectedMode)}function requestFire(e){e.preventDefault();game.audio.unlock();input.fire=true}document.getElementById('startButton').addEventListener('click',start);document.querySelectorAll('.mode-card[data-mode]').forEach(function(card){card.addEventListener('click',function(){selectedMode=card.dataset.mode;document.querySelectorAll('.mode-card[data-mode]').forEach(function(item){item.classList.toggle('is-selected',item===card)});game.mode=selectedMode;game.updateRecordLabel();})});document.querySelectorAll('.shop-card[data-upgrade]').forEach(function(card){card.addEventListener('click',function(){game.buyUpgrade(card.dataset.upgrade)})});document.getElementById('shopContinueButton').addEventListener('click',function(){game.audio.unlock();game.continueOrRestart()});document.getElementById('resultButton').addEventListener('click',function(){game.audio.unlock();game.continueOrRestart()});document.getElementById('resetButton').addEventListener('click',function(){game.state='menu';game.syncUiState();input.fire=false;input.keys={};input.pointerMoved=false;input.aimActive=false;input.aimMode='pointer';if(input.resetAimStick)input.resetAimStick();document.getElementById('statusText').textContent='准备就绪';game.updateHud();document.getElementById('menuPanel').hidden=false;document.getElementById('resultPanel').hidden=true;document.getElementById('shopPanel').hidden=true;document.getElementById('pauseButton').disabled=true;document.getElementById('fireTopButton').disabled=true});document.getElementById('pauseButton').addEventListener('click',function(){if(game.state==='playing'){game.state='paused';game.syncUiState();this.textContent='继续';document.getElementById('statusText').textContent='已暂停'}else if(game.state==='paused'){game.state='playing';game.syncUiState();this.textContent='暂停';document.getElementById('statusText').textContent='战斗中'}});document.getElementById('soundButton').addEventListener('click',function(){var muted=game.audio.setMuted(!game.audio.muted);this.textContent=muted?'音效关':'音效开';this.setAttribute('aria-pressed',String(muted));if(!muted)game.audio.unlock()});document.querySelectorAll('[data-key]').forEach(function(btn){var key=btn.dataset.key;['pointerdown','touchstart'].forEach(function(ev){btn.addEventListener(ev,function(e){e.preventDefault();input.keys[key]=true})});['pointerup','pointercancel','touchend','pointerleave'].forEach(function(ev){btn.addEventListener(ev,function(e){e.preventDefault();input.keys[key]=false})})});document.getElementById('fireButton').addEventListener('pointerdown',requestFire);document.getElementById('fireTopButton').addEventListener('pointerdown',requestFire)})();
